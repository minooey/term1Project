...
# The ROS side of assignment B  is done here, please note there is 2 option in unity UI
#and this file is corresponds to freedom mode.
# please write the set_pos in Unity Int32 publisher Topic to connect
...
class Controller:
    ...
    def __init__(self):
        ...
        # Ros variables
        with self.mutex:
            rospy.Subscriber("set_pos", Int32, self.callback, queue_size=10, buff_size=2 ** 24)
            self.set_pos = 0
            rospy.Subscriber("set_pos2", Int32, self.callback1, queue_size=10, buff_size=2 ** 24)
            self.set_pos2 = 0
            rospy.Subscriber("set_pos3", Int32, self.callback2, queue_size=10, buff_size=2 ** 24)
            self.set_pos3 = 0
    ...
    def dxl_torque_disable(self):
        ...
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID[0],
                                                                       self.ADDR_PRO_TORQUE_ENABLE, TORQUE_DISABLE)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def callback(self, data):
            self.set_pos = data.data

    def callback1(self, data):
            self.set_pos2 = data.data

    def callback2(self, data):
            self.set_pos3 = data.data
